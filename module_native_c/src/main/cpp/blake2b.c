...
int blake2b_Key(const uint8_t *msg, uint32_t msg_len, const void *key, size_t keylen, void *out,
                size_t outlen) {
    BLAKE2B_CTX ctx;
...
Java_com_lxx_nativec_Blake2bUtil_blake2b(JNIEnv *env,jclass type,jbyteArray msg_,jint length) {
...
//    //创建输出的数组
    jbyte out[length];
    //获取输入数组长度
    jsize bytesLen = (*env)->GetArrayLength(env, msg_);
    //执行blake2b方法并输出数组
    blake2b(msg, bytesLen, out, length);
    //回收输入数组
    (*env)->ReleaseByteArrayElements(env, msg_, msg, 0);

    jbyteArray jarray = (*env)->NewByteArray(env, length);
    (*env)->SetByteArrayRegion(env, jarray, 0, length, out);
    return jarray;
}